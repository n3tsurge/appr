...
async function fetchData(p: number) {
  loading.value = true
  try {
    const res = await apiGet<PaginatedResponse<Component>>(`/components?page=${p}&per_page=${perPage}`)
    rows.value = res.data
...
<template>
  <AppLayout>
    <template #header-title>
      <h1 class="text-xl font-semibold text-gray-800">Components</h1>
    </template>

    <div>
      <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Components</h2>
        <p class="mt-1 text-sm text-gray-500">{{ total }} total components</p>
      </div>
      ...
      >
        ...
        <template #owner_team="{ row }">
          {{ (row as unknown as Component).owner_team?.name ?? '-' }}
        </template>
      </DataTable>
